...
-(void)cutOffSocket
{
    self.socket.userData = SocketOfflineByUser1;
...
- (void)sendMessage:(id)message
{
    ...
    [self.socket writeData:cmdData withTimeout:WRITE_TIME_OUT tag:0];
}
...
- (void)onSocketDidDisconnect:(AsyncSocket *)sock
{
    ...
    if (sock.userData == SocketOfflineByServer1) {
        // 服务器掉线，重连
        [self startConnectSocket];
    }
    else if (sock.userData == SocketOfflineByUser1) {
        
        // 如果由用户断开，不进行重连
        return;
    }else if (sock.userData == SocketOfflineByWifiCut1) {
...
- (void)onSocket:(AsyncSocket *)sock willDisconnectWithError:(NSError *)err
{
    ...
    if(unreadData.length > 0) {
        ...
        if (err.code == 57) {
            self.socket.userData = SocketOfflineByWifiCut1;
        }
    }
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if( data.length < MAX_BUFFER )
    {
        ...
        if([_delegate respondsToSelector:@selector(requestDataReturn:)]){
            [_delegate requestDataReturn:string];
        }
    }
